...
export class EnergyComponent implements OnInit {
  ...
  public energyType: string = 'conuscddresults'
  public energyDisplayValue: string = 'Cooling Degree Days';
  date;
  chartData;
  public canvas: any;
  public ctx;
  ...
  constructor(private appService: AppService) {
    ...
    this.appService.getToken().subscribe((res: any) => {
      ...
      this.appService.getForecast(data, this.energyType).subscribe((res: any) => {
        this.energyData = res
  ...
  updateEnergyType(type, displayName) {
    this.energyType = type;
    this.energyDisplayValue = displayName;
    let data = {
      date: this.date,
      longitude: this.lng,
      latitude: this.lat
  ...
  forecastCategoryFunc() {
    console.log(this.energyData['forecast category'])
    switch (this.energyData['forecast category']) {
      case '1.0':
        return `Much Below Normal ${this.energyDisplayValue} Activity Expected`
        break;

      case '2.0':
        return `Below Normal ${this.energyDisplayValue} Activity Expected`

        break;
      case '3.0':
        return `Average Normal ${this.energyDisplayValue} Activity Expected`

        break;
      case '4.0':
        return `Above Average Normal ${this.energyDisplayValue} Activity Expected`

        break;
      default:
        return `Much Above Average Normal ${this.energyDisplayValue} Activity Expected`
